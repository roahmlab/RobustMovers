...
def integrate(model, ts_sim, x0, desired_trajectory, controller, method='RK45'):
    ...
    controller : a controller object with an update function
        A controller object that takes the current state (q, v) and desired trajectory (qd, qd_d, qd_dd)
        and returns the control input (torques).
    method : str, optional
    ...
    for i, t in enumerate(ts_sim):
        ...
        v = vs[i]
        qd, qd_d, qd_dd = desired_trajectory(t)
        taus[i] = controller.update(q, v, qd, qd_d, qd_dd)
